...
fn validate_custom_url(url: &str) -> Result<String> {
    ...
    let parsed = url::Url::parse(url)
        .map_err(|e| SolanaMcpError::InvalidEndpoint(e.to_string()))?;

    if let Some(host) = parsed.host_str()
        && (host == "localhost" || host.starts_with("127.") || host.starts_with("10.")
...
mod tests {
    ...
    fn test_default_is_devnet() {
        unsafe { env::remove_var("SOLANA_NETWORK"); }
    ...
    fn test_mainnet_parsing() {
        unsafe { env::set_var("SOLANA_NETWORK", "mainnet"); }
        let config = Config::from_env().unwrap();
    ...
    fn test_custom_url_rejected_if_not_https() {
        unsafe { env::set_var("SOLANA_NETWORK", "http://evil.com"); }
    ...
    fn test_private_ip_rejected() {
        unsafe { env::set_var("SOLANA_NETWORK", "https://127.0.0.1:8899"); }
        let result = Config::from_env();
        assert!(result.is_err());
        unsafe { env::remove_var("SOLANA_NETWORK"); }
    }
